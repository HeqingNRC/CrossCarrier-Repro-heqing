...
OUT = Path(os.environ["XC_REPRO_OUTPUT"]) / "ensemble" if os.environ.get("XC_REPRO_OUTPUT") else REV / "SUPP_ABLATION" / "eval_out"
...
@torch.no_grad()
def get_logits(model, x, bs=None):
    bs = bs or portable.batch_size()
    out = []
    for i in range(0, x.shape[0], bs):
        with portable.autocast(DEVICE):
            z = model.encode_adapted(x[i:i + bs].to(DEVICE))
...
def main():
    ...
    x = cache(full)
    print(f"[data] full-418={len(full)} precision={portable.precision(DEVICE)} batch={portable.batch_size()}", flush=True)
    ...
    for name, cfg, subdir in CONFIGS:
        ...
        for s in SEEDS:
            ...
            if ck_path is None:
                ...
                continue
            portable.load_adapter(model, ck_path)
            print(f"[eval] {cfg} seed={s}", flush=True)
