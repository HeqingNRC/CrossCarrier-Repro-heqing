...
class HighFreqDopplerTexturize:
    ...
    def __call__(self, img):
        ...
        (h, w, _) = arr.shape
        # New runner opts into the global NumPy RNG seeded by set_seed/workers.
        # Preserve the author's legacy default for all existing entry points.
        rng = np.random if getattr(config, 'SEEDED_HFT', False) else np.random.default_rng()
