...
def amp_dtype():
    return {"bf16": torch.bfloat16, "fp16": torch.float16,
            "fp32": torch.float32}[config.AMP_DTYPE]
...
def build_source_only_dataloaders(log, run_dir, seed, device):
    """Strict fixed-epoch protocol: no target manifest or target loader access."""
    train_df = load_manifest("train", keep_7c=True)
    val_df = load_manifest("val", keep_7c=True)
    for split, df in (("train", train_df), ("val", val_df)):
        if set(df["frequency"]) != set(config.TRAIN_FREQS):
            raise ValueError(f"{split}: expected source frequencies {config.TRAIN_FREQS}")
        if set(df["class"]) != set(config.CLASSES):
            raise ValueError(f"{split}: missing source classes")
        if (df["freq_idx"] < 0).any():
            raise ValueError(f"{split}: unknown source domain")
    if set(train_df["path"]) & set(val_df["path"]):
        raise ValueError("train and source validation paths overlap")
    manifest_dir = run_dir / "manifests"
    manifest_dir.mkdir(parents=True, exist_ok=True)
    train_df.to_csv(manifest_dir / "train.csv", index=False)
    val_df.to_csv(manifest_dir / "val.csv", index=False)
    loader = (GpuSourceProvider(train_df, seed, device, log=log)
              if config.USE_FAST_GPU else make_source_train_loader(train_df, seed=seed))
    log(f"train_source={len(train_df)} val_source={len(val_df)} "
        f"frequencies={config.TRAIN_FREQS}; target not loaded")
    return {"train_source": loader, "train_source_df": train_df, "val_all_df": val_df}
...
def train(args, run_dir, device, log):
    strict_final = bool(getattr(args, "strict_final_ema", False))
    if strict_final and getattr(args, "resume", None):
        raise NotImplementedError(
            "Strict final-EMA runs do not support resume: the legacy bundle omits "
            "carrier-adversary, scaler and sampler state. Start a fresh run.")
    set_seed(args.seed)
    log(f"set_seed({args.seed})")
    build_loaders = build_source_only_dataloaders if strict_final else build_dataloaders
    loaders = build_loaders(log, run_dir=run_dir, seed=args.seed, device=device)
    ...
    if steps_per_epoch <= 0:
        raise ValueError("No full source batch; decrease --batch-size or fix source manifests")
    ...
    if strict_final:
        log("selection=fixed_final_epoch_ema; no source/target checkpoint selection or epoch evaluation")
    else:
        log(
    ...
    pool_state = None if strict_final else pool_protocol.init_pool(run_dir, log_fn=log)
    ...
    for ep in range(start_epoch, args.epochs + 1):
        ...
        for step in range(steps_per_epoch):
            ...
            if strict_final and not torch.isfinite(loss_source):
                raise FloatingPointError(f"Non-finite loss at epoch {ep}, step {step + 1}")
            ...
            if ep >= config.EMA_START_EPOCH and optimizer_updated:
                ema.update(model)
        ...
        if strict_final:
            # This branch cannot inspect target loaders, run pool selection, or
            # encode missing metrics as zero. Selection is fixed before training.
            row = {
                "epoch": ep, "time_s": time.time() - t0,
                "steps": n_batches, "global_step": global_step,
                "optimizer_updates": epoch_optimizer_steps,
                "fp16_overflow_skips": n_batches - epoch_optimizer_steps,
                ...
                "grl_lambda": grl_lambda_last, "lr_end": current_lr(opt),
                "das_stage": das_stage,
                "source_validation": None, "target_evaluation": None,
                "smoke_only": bool(max_steps is not None),
            }
            hist.append(row)
            (run_dir / "reports" / "history.json").write_text(
                json.dumps(hist, indent=2), encoding="utf-8")
            log(f"[ep_summary] ep={ep}/{args.epochs} train_acc={row['train_acc']:.4f} "
                f"loss_ce={row['loss_ce']:.4f} loss_grl={row['loss_grl']:.4f}")
            if ep == args.epochs:
                if optimizer_steps == 0:
                    raise FloatingPointError("No optimizer updates completed; increase smoke --max-steps or use fp32")
                final_path = run_dir / "checkpoints" / "final_ema.pt"
                atomic_save({
                    "format": "cross_frequency_final_ema_v1",
                    "ema": {k: v.detach().to("cpu", torch.float32).clone()
                            for k, v in ema.state_dict().items()},
                    "backbone": args.backbone, "adapter_mode": args.adapter_mode,
                    "epoch": ep, "seed": args.seed,
                    "protocol": args.protocol, "variant": args.variant,
                    "scientific_config_sha256": args.scientific_config_sha256,
                    "source_manifest_hashes": args.source_manifest_hashes,
                    "source_frequencies": list(config.TRAIN_FREQS),
                    "target_frequencies": list(config.TEST_FREQS),
                    "classes": list(config.CLASSES),
                    "selection": "fixed_final_epoch_ema",
                    "target_used_for_training_or_selection": False,
                    "precision": config.AMP_DTYPE,
                    "smoke_only": bool(max_steps is not None),
                    "ema_start_epoch": config.EMA_START_EPOCH,
    ...
    if strict_final:
        total_bar.close()
        log(f"[train] done final_epoch={args.epochs}; target has not been loaded")
        return {"final_ema": final_path, "model": model, "ema": ema, "loaders": loaders}
