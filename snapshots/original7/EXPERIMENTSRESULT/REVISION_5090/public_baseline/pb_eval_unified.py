...
def proposed_forward(ckpt_path):
    ...
    ck = P.portable.load_adapter(model, ckpt_path)
...
def softmax_posteriors(forward_fn, batches):
    ...
    with torch.no_grad():
        for x, _y, _d in batches:
            with P.portable.autocast(P.DEVICE):
                lg = forward_fn(x.to(P.DEVICE))
...
def main():
    ...
    output_dir = Path(os.environ["XC_REPRO_OUTPUT"]) / "unified" if os.environ.get("XC_REPRO_OUTPUT") else PB
    output_dir.mkdir(parents=True, exist_ok=True)
    out_json = output_dir / f"{args.out_prefix}.json"
    out_md = output_dir / f"{args.out_prefix}_auto.md"
