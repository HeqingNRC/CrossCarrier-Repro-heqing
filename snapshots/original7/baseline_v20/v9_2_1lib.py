...
class CurriculumSourceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Domain indices are local to the selected pair of physical carriers.
        # Do not reuse a stale 10/24-domain index from a manifest prepared for
        # another transfer direction.
        d = config.FREQ_TO_IDX.get(row["frequency"], -1)
        if d < 0:
            raise ValueError(f"Target or unknown carrier in source data: {row['frequency']}")
...
def _build_img_cache(df, device):
    ...
    for i in range(n):
        ...
        di = config.FREQ_TO_IDX.get(row["frequency"], -1)
        ds[i] = di
